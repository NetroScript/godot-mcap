...
impl MCAPReplay {
	fn update_replay(&mut self) {
		...
		let Some(start_rt) = self.start_real_time else { return; };
		let Some(start_lt) = self.start_log_time else { return; };
		...
		if let Some(t_end) = self.time_end { if target > t_end { target = t_end; } }

		// Collect messages to emit up to target time
		let mut to_emit: Vec<Gd<MCAPMessage>> = Vec::new();
		enum EndAction { None, Stop, Restart }
		let mut action = EndAction::None;

		loop {
			let next_opt = {
				match self.iter.as_mut() { Some(it) => it.bind_mut().peek_message(), None => None }
			};
			let Some(next) = next_opt else {
				// End of stream
				if self.looping { action = EndAction::Restart; } else { action = EndAction::Stop; }
				break;
			};
			...
			if let Some(t_end) = self.time_end { if msg_time > t_end { action = if self.looping { EndAction::Restart } else { EndAction::Stop }; break; } }
			...
			if msg_time <= target {
				...
				if let Some(chset) = &self.filter_channels {
					...
					if !chset.contains(&ch_id) {
						// consume and skip
						let _ = { if let Some(it) = self.iter.as_mut() { it.bind_mut().get_next_message() } else { None } };
						continue;
					}
				}

				// consume and emit
				let msg_opt = { if let Some(it) = self.iter.as_mut() { it.bind_mut().get_next_message() } else { None } };
	...
	fn setup_iterator(&mut self, start_time: Option<u64>) -> bool {
		let reader = match &self.reader { Some(r) => r.clone(), None => return false };
		// Build a fresh iterator from reader
		let mut it = reader.bind().stream_messages_iterator();
		// Fast-path single-channel filter
		if let Some(set) = &self.filter_channels { if set.len() == 1 { if let Some(&cid) = set.iter().next() { it.bind_mut().for_channel(cid as i32); } } }
		if let Some(t) = start_time { let _ = it.bind_mut().seek_to_time(t as i64); }
		self.iter = Some(it);
		true
	}
	...
	fn restart_from_range_start(&mut self) {
		...
		if let Some(s) = self.time_start { start_t = s; } else if let Some(r) = &self.reader { start_t = r.clone().bind_mut().first_message_time_usec().max(0) as u64; }
		if !self.setup_iterator(Some(start_t)) { self.stop(); return; }
...
impl INode for MCAPReplay {
	fn on_notification(&mut self, what: NodeNotification) {
		if what == NodeNotification::INTERNAL_PROCESS && self.processing_mode == ProcessingMode::IDLE {
			self.update_replay();
		} else if what == NodeNotification::INTERNAL_PHYSICS_PROCESS && self.processing_mode == ProcessingMode::PHYSICS {
			self.update_replay();
		}
	}
}
...
impl MCAPReplay {
	...
	pub fn set_filter_channels(&mut self, channel_ids: PackedInt32Array) {
		let mut set: HashSet<u16> = HashSet::new();
		for i in 0..channel_ids.len() { if let Some(v) = channel_ids.get(i) { if v >= 0 { let _ = set.insert(v as u16); } } }
		self.filter_channels = if set.is_empty() { None } else { Some(set) };
		// Rebuild iterator at current logical time if running
		if self.running { let now = self.current_time_usec(); self.setup_iterator(Some(now.max(0) as u64)); self.start_log_time = Some(now.max(0) as u64); self.start_real_time = Some(Instant::now()); }
	}

	/// Clear channel filter.
	#[func]
	pub fn clear_filter_channels(&mut self) { self.filter_channels = None; if self.running { let now = self.current_time_usec(); self.setup_iterator(Some(now.max(0) as u64)); self.start_log_time = Some(now.max(0) as u64); self.start_real_time = Some(Instant::now()); } }
	...
	pub fn set_time_range(&mut self, start_usec: i64, end_usec: i64) {
		self.time_start = if start_usec >= 0 { Some(start_usec as u64) } else { None };
		self.time_end = if end_usec >= 0 { Some(end_usec as u64) } else { None };
		if self.running { self.restart_from_range_start(); }
	}

	/// Start replay. If a time range start is set, starts from there, else from file's first message time.
	#[func]
	pub fn start(&mut self) -> bool {
		if self.reader.is_none() { return false; }
		// Determine start time
		let mut start_t: u64 = 0;
		if let Some(s) = self.time_start { start_t = s; } else if let Some(r) = &self.reader { start_t = r.clone().bind_mut().first_message_time_usec().max(0) as u64; }
		if !self.setup_iterator(Some(start_t)) { return false; }
		self.start_log_time = Some(start_t);
		self.start_real_time = Some(Instant::now());
		self.running = true;
		self.apply_process_state();
	...
	#[func]
	pub fn seek_to_time(&mut self, log_time_usec: i64) -> bool {
		let t = if log_time_usec < 0 { 0 } else { log_time_usec as u64 };
		if self.iter.is_none() && !self.setup_iterator(Some(t)) { return false; }
		if let Some(it) = &mut self.iter { if !it.bind_mut().seek_to_time(log_time_usec) { return false; } }
	...
	pub fn current_time_usec(&self) -> i64 {
		match (self.start_log_time, self.start_real_time) {
			(Some(sl), Some(sr)) => {
				...
				if let Some(e) = self.time_end { if cur as u64 > e { cur = e as i64; } }
				cur
			}
			_ => -1,
		}
	...
	#[func]
	pub fn set_speed(&mut self, speed: f64) { self.speed = if speed <= 0.0 { 1.0 } else { speed }; }

	/// Set processing mode to use either idle or physics ticks.
	#[func]
	pub fn set_processing_mode(&mut self, mode: ProcessingMode) { self.processing_mode = mode; self.apply_process_state(); }
